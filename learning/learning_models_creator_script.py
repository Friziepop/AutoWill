...
def get_best_model(start_deg: int, end_deg: int, material_id: int, feature, models_dir: str, show_graph: bool = False):
    ...
    for deg in range(start_deg, end_deg + 1):
        current_model = learn_poly(degree=deg, material_id=material_id, feature=feature,
                                   ...
                                   show_graph=False)
        current_score = current_model.score(csv_data_path=get_vars_path(mode='v'), x_cols=X_COLS, y_col=feature)
...
def learn_models(materials_ids: List[int], path: str = CSV_DIR, models_dir: str = MODELS_DIR,
    ...
    for mat_id in materials_ids:
        ...
        coeff_quarter = quarter_learner.train(get_vars_path(mode='a'), x_cols=X_COLS, y_col="quarter",
                                            save_model=True)
